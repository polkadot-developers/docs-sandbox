...
[state transition function](/v3/concepts/runtime) must be deterministic.
...
[extrinsics](/v3/concepts/extrinsics). The header must contain a reference to its parent block
...
[`SelectChain` Trait](/rustdocs/latest/sp_consensus/trait.SelectChain.html).
...
[`LongestChain` struct](/rustdocs/latest/sc_consensus/struct.LongestChain.html).
...
<Message
  type={`gray`}
  title={`Note`}
  text={`Some consensus systems couple block production and finality, as in, finalization is part of the
...
[Aura](/rustdocs/latest/sc_consensus_aura/index.html) provides a slot-based
block authoring mechanism. In Aura a known set of authorities take turns producing blocks.

### BABE

[BABE](/rustdocs/latest/sc_consensus_babe/index.html) also provides slot-based
...
[Proof-of-work](/rustdocs/latest/sc_consensus_pow/index.html) block authoring
...
[GRANDPA](/rustdocs/latest/sc_finality_grandpa/index.html) provides block
...
[`DigestItem`](/rustdocs/latest/sp_runtime/enum.DigestItem.html), a message
