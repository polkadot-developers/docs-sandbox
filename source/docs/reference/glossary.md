...
[chain specification](/v3/runtime/chain-specs) file.
...
As a canary network, Kusama is [controlled by its network participants](/v3/runtime/frame#democracy) is intended to be stable enough to encourage meaningful experimentation.
...
The [metadata](/v3/runtime/metadata) that exposes information about a Substrate [blockchain](#blockchain) enables you to interact with that system. 
...
The FRAME `system` module defines [three built-in origins](/v3/runtime/origins#raw-origins). 
...
This trie is exposed to the [runtime](#runtime) as [a simple key/value map](/v3/advanced/storage) where both keys and values can be arbitrary byte arrays.
...
[runtime](#runtime). 
Learn more about storage items in this article about [runtime storage](/v3/runtime/storage).
...
A type of [extrinsic](#extrinsic) that can be safely gossiped between [nodes](#node) on the network because it can be verified through [signatures](/v3/concepts/extrinsics#signed-transactions) or [signed extensions](/v3/concepts/tx-pool#signed-extension).
